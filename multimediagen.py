...
REPLICATE_VIDEO_MODEL_ENDPOINT = st.secrets["REPLICATE_VIDEO_MODEL_ENDPOINT"]
...
def generate_content_with_examples(institution, page_type, examples, facts, writing_styles, style_weights, keywords, audience, specific_facts_stats, min_chars, max_chars, image_description, video_description):
    examples_text = "\n\n".join(examples)
    prompt = (
    ...
    image_url = image_output[0] if image_output else None

    # Generate video using the video_description
    video_output = replicate.run(
        REPLICATE_VIDEO_MODEL_ENDPOINT,
        input={
    ...
    return generated_text, image_url, video_url
...
def main():
    ...
    if uploaded_file and st.button("Generate Content"):
        ...
        if file_urls:
            ...
            for _, row in csv_data.iterrows():
                ...
                video_description = row.get("Video", "")
                
                generated_content, image_url, video_url = generate_content_with_examples(
                    institution, page_type, examples, specific_facts_stats, writing_styles, style_weights, keywords, audience, specific_facts_stats, min_chars, max_chars, image_description, video_description
                )
                generated_pages.append((institution, page_type, generated_content, image_url, video_url))
    ...
    if st.session_state.generated_pages:
        for idx, (institution, page_type, content, image_url, video_url) in enumerate(st.session_state.generated_pages):
            st.subheader(f"{institution} - {page_type}")
            st.text_area("Generated Content", content, height=300)
            if image_url:
                st.image(image_url, caption="Generated Image", use_column_width=True)
            if video_url:
                st.video(video_url, caption="Generated Video")
